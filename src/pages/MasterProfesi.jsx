...
import {
  getprofesiall, postprofesi, updateprofesi, deleteprofesi
} from "../services/MasterProfesi";
...
const MasterProfesi = () => {
  ...
  const toggleModal = () => { setUuid(""); setName(""); setModalOpen(!modalOpen); };
  ...
  const handleDelete = async (row) => {
    ...
    try {
      const response = await deleteprofesi(row.uuid, accessToken);
  ...
  const handleGetProfesiAll = async () => {
    try {
      const response = await getprofesiall(accessToken);
      setDataProfesi(response.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (uuid) {
        response = await updateprofesi(data, accessToken);
      } else {
        response = await postprofesi(data, accessToken);
      }
    ...
    <Container className="my-4 bg-white rounded shadow p-4">
      <Row className="align-items-center mb-3">
        <Col xs="10" md="6" className="d-flex align-items-center">
          <h4 className="fw-bold text-dark mb-0">List Master Profesi</h4>
          <Button
            color="danger"
      ...
      <Modal isOpen={modalOpen} toggle={toggleModal} size="lg" modalClassName="custom-modal">
        <ModalHeader toggle={toggleModal}>Data Profesi ( Entri / Edit )</ModalHeader>
        <ModalBody>
