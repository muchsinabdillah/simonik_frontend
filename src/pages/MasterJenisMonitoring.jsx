...
import {
  getjenismonitoringall, postjenismonitoring, updatejenismonitoring, deletejenismonitoring
} from "../services/MasterJenisMonitoring";

import { toast, ToastContainer } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";

const MasterJenisMonitoring = () => {
  ...
  const handleDelete = async (row) => {
    ...
    try {
      const response = await deletejenismonitoring(row.uuid, accessToken);
      toast.success(response.message || "Data berhasil dihapus");
      handlegetjenismonitoringall();
    } catch (error) {
  ...
  const handlegetjenismonitoringall = async () => {
    try {
      const response = await getjenismonitoringall(accessToken);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (uuid) {
        response = await updatejenismonitoring(data, accessToken);
      } else {
        response = await postjenismonitoring(data, accessToken);
      }
    ...
    <Container className="my-3 bg-white rounded shadow p-4">
      ...
      <Row className="align-items-center mb-3">
        <Col xs="10" md="6" className="d-flex align-items-center">
          <h4 className="fw-bold text-dark mb-0">List Master Jenis Monitoring</h4>
          <Button
            color="danger"
      ...
      <Modal isOpen={modalOpen} toggle={toggleModal} size="lg" modalClassName="custom-modal">
        <ModalHeader toggle={toggleModal}>Data Jenis Monitoring ( Entri / Edit )</ModalHeader>
        <ModalBody>
          <form onSubmit={(e) => handleSubmit(e)}>
            <Row>
              <Col md={12}>
                ...
                <div className="mb-3">
                  <Row className="align-items-center">
                    <Col md={4}><label htmlFor="name" className="form-label mb-0">Nama Jenis Monitoring:</label></Col>
                    <Col md={8}>
                      <Input
                        type="text"
                        value={name}
                ...
                <div className="mb-3">
                  <Row className="align-items-center">
                    <Col md={4}><label htmlFor="status" className="form-label mb-0">Status Jenis Monitoring:</label></Col>
                    <Col md={8}>
                      <Input
                        type="select"
                        value={status}
...
export default MasterJenisMonitoring;
